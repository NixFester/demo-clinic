...
export default function FollowupPage() {
  ...
  const fetchData = async (p = page) => {
    ...
    try {
      ...
      console.error('[FollowupPage] Error:', err);
  ...
  const fetchPasien = async () => {
    try {
      ...
      console.error('[FollowupPage] Pasien error:', err);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <Button onClick={() => { setForm({ id_pasien: '', jenis_followup: 'konfirmasi', pesan: '' }); setDialogOpen(true); }} className="bg-emerald-600 hover:bg-emerald-700">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {form.id_pasien && form.pesan && (
                <div className="p-3 bg-green-50 rounded-lg">
                  ...
                  >
                    {generateWALink(pasienList.find(p => p.id === parseInt(form.id_pasien))?.no_whatsapp || '', form.pesan).substring(0, 80)}...
                  </a>
                </div>
              )}
        ...
        <CardContent>
          {loading ? (
            ...
            <Table>
              ...
              <TableBody>
                {data.length === 0 ? (
                  <TableRow><TableCell colSpan={6} className="text-center py-8 text-gray-500">Tidak ada follow-up</TableCell></TableRow>
                ) : (
                  data.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell><StatusBadge status={item.status === 'terkirim' ? 'final' : 'menunggu'} /></TableCell>
                      <TableCell>{item.tanggal_kirim ? formatDateTime(item.tanggal_kirim) : '-'}</TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          {item.no_whatsapp && (
                            <Button size="sm" variant="outline" asChild>
                              <a href={generateWALink(item.no_whatsapp, item.pesan)} target="_blank" rel="noopener noreferrer">
